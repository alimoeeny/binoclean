...
int twomonitors = 1;
//Ali #else
//Ali static int useDIO = 0;
//Ali int twomonitors = 0;
//Ali #endif
...
//Ali #if defined(Linux) || defined(Cygwin)
//Ali static char theport[256] = "/dev/ttys2";
//Ali #elif defined(macosx)
static char theport[256] = "/dev/tty.KeySerial1";
//Ali #else
//Ali static char theport[256] = "/dev/ttyd2";
//Ali #endif
static char *statusstring = NULL;
//Ali #ifdef macosx
char stepperport[256] = "/dev/ttys1";
//Ali #else
//Ali char stepperport[256] = "/dev/ttyf1";
//Ali #endif
...
XColor forecolor,backcolor,curscolor;
Colormap cmap;
...
void cylinder_proc(Widget w, int val, int data); /*j*/
...
/* BCNEW - no  longer a static*/
MenuItem stimulus_menu[] = {
/* end BCNEW */
	{ "Background", set_stimulus, "back", (MenuItem *)NULL },
	{ "Gabor", set_stimulus, "gabor", (MenuItem *)NULL },
	{ "Sine", set_stimulus, "sinewave", (MenuItem *)NULL },
	{ "RDS", set_stimulus, "rds", (MenuItem *)NULL },
	{ "Bar", set_stimulus, "bar", (MenuItem *)NULL },
	{ "Cylinder", set_stimulus, "cylinder", (MenuItem *)NULL }, /*j*/
	{ "2xSine", set_stimulus, "dblsine", (MenuItem *)NULL },
	{ "Corrug", set_stimulus, "corrug", (MenuItem *)NULL },
	{ "None", set_stimulus, "none", (MenuItem *)NULL },
	{ "Square", set_stimulus, "squre", (MenuItem *)NULL },
	{ "Probe", set_stimulus, "probe", (MenuItem *)NULL },
	{ "Test", set_stimulus, "test", (MenuItem *)NULL },
	{ "Options", set_stimulus, "options", (MenuItem *)NULL },
	{ "Optimize", set_stimulus, "optimize", (MenuItem *)NULL },
	{ "Panel", set_stimulus, "panel", (MenuItem *)NULL },
	{ "Save Stim as..", set_stimulus, "load", (MenuItem *)NULL },
	{ "Load StimFile", set_stimulus, "Write", (MenuItem *)NULL },
	{ "Save", set_stimulus, "save", (MenuItem *)NULL },
	NULL,
};

MenuItem expt_menu[] = {
	{ "All", set_stimulus, "all", (MenuItem *)NULL },
	{ "Expt", set_stimulus, "expt", (MenuItem *)NULL },
	{ "Wurtz", set_stimulus, "wurtz", (MenuItem *)NULL },
	{ "AFC", set_stimulus, "afc", (MenuItem *)NULL },
	{ "BW", set_stimulus, "bw", (MenuItem *)NULL },
	{ "Log", set_stimulus, "log", (MenuItem *)NULL },
	{ "Unit Log", set_stimulus, "unit", (MenuItem *)NULL },
	NULL,
};
...
MenuItem run_menu[] = {
	{ "Run Continuously", set_run, "cont", (MenuItem *)NULL },
	{ "Run Once", set_run, "rgbdouble", (MenuItem *)NULL },
	{ "Run a Set", set_run, "runset", (MenuItem *)NULL },
	{ "Reset Window", set_run, "reset", (MenuItem *)NULL },
	{ "Reset Counters", set_run, "restart", (MenuItem *)NULL },
	{ "Kill Window", set_run, "kill", (MenuItem *)NULL },
	{ "Step", set_run, "step", (MenuItem *)NULL },
	{ "Freeze", set_run, "freeze", (MenuItem *)NULL },
	{ "Monocular ", set_run, "monoc", (MenuItem *)NULL },
	{ "6 Bit double ", set_run, "color6dbl", NULL },
	{ "Run RectCopy", set_run, "copy", (MenuItem *)NULL },
	{ "Run Test", set_test_loop, "test", (MenuItem *)NULL },    
	NULL,
};


static Sliderdat allsliderprops[] ={
	...
	{ 0.0, 10.0, 2, SD_Y,"Sigma Y"},
	{ 0.0, 10.0, 2, STIM_SIZE,"Size"},
	{ 0.0, 10.0, 2, SF2,"SF2 (cpd)"},
	{ 0.0, 10.0, 2, TF2,"TF2 (Hz)"},
	{ -1.0, 1.0, 3, CONTRAST2,"Contrast 2"},
	{ 0.0, 360.0, 0, PHASE2,"Phase off"},
	{ 0.0, 1.0, 2, SETBACKCOLOR,"Back Color"},
	{ 0.0, 1.0, 2, SETOVERLAYCOLOR,"Overlay Color"},
	{ 0.0, 10.0, 2, SETZXOFF,"X offset"},
	{ 0.0, 10.0, 2, SETZYOFF,"Y offset"},
	{ -10.0, 10.0, 1, FIXPOS_X,"Fixation X"},
	{ -10.0, 10.0, 1, FIXPOS_Y,"Fixation Y"},
	{ 0.0, 50.0, 0, DOT_DENSITY,"Dot Density"},
	{ 0.0, 50.0, 2, DOT_SIZE,"Dot Size"},
	{ 0.0, 50.0, 0, XSAMPLES,"X Sampling"},
	{ 0.0, 50.0, 2, YSAMPLES,"Y Sampling"},
	{ 0.0, 360., 0, SETPHASE,"Phase"},
	{ 0.0, 50.0, 0, XMINSAMPLE_RATIO,"X Sample/cycle"},
	{ 0.0, 50.0, 0, XMAXSAMPLE_RATIO,"X Sample Limit"},
	{ 0, 1.0, 2, SETFIXCOLOR,"Fix Color"},
	{ 0, 40.0, 3, SETFIXSIZE,"Fix Size"},
	{ 0, 40.0, 0, FIX_OFF_CODE,"Fix Off Color"},
	{ 0.0, 600., 0, TEST_BIT,"Test"},
	{ 0.0, 8.00, 2, SETZOOM,"Zoom"},
	{ 0.0, 2.00, 3, GAMMAVAL_CODE,"Gamma"},
	NULLPROP,
};
...
//Ali #else
//Ali 	"*fontList: -*-times-*-r-*--10-*=charset1",
//Ali 	"*HelpWin*fontList: -*-times-*-r-*--14-*=charset1",
//Ali 	"*HelpWin*height: 500",
//Ali #endif
...
extern Widget allclock,allstatuswidget,allframe,stepform;
extern int frameiseqp[];


Widget toplevel,statuswidget,stimbar,fsdialog = 0, clockwidget = 0, oform = 0;
Widget sfsdialog = 0,otoggle_box = 0;
static Widget lform = 0, form = 0;
extern Widget exframe, wframe, bwframe,loframe;
...
void SetWidgetTimes()
{
  ...
  if(timediff(&now,&timeb) > 1.0)
    {
  SetWidgetTime(clockwidget);
  SetWidgetTime(allclock);
  memcpy(&timeb,&now, sizeof(struct timeval));
  if(monkeypress == WURTZ_STOPPED    && expt.vals[GRIDSIZE] < 0.1){
    x[0] = 0;
    x[1] = 100;
...
float GetFrameRate(void)
{
  ...
  command = "/usr/gfx/gfxinfo|grep \"Video Output\"";
  p = popen(command, "r");
  n = fscanf(p, "%*s %*s %d pixels, %d lines, %fHz", &pixels, &lines, &rate);
  pclose(p);

  command = "/usr/sbin/gamma";
  p = popen(command, "r");
  fgets(buf, BUFSIZ, p);
  sscanf(buf, "%f", &gamma);
  expt.mon->truegamma = gamma;
  pclose(p);
  if(fabsf(gamma-expt.mon->gamma) > 0.05){
    sprintf(buf,"Gamma (%.3f) Not set to %.3f for %s",gamma,expt.mon->gamma,expt.mon->name);
    acknowledge(buf,NULL);
  }
...
void SetWidgetColor(float color)
{
	...
	ival = color * 255;
	if(optionflag & LEFT_FIXATION_CHECK && !optionflags[GREY_MONOC])
        forecolor.blue = 0;
	else
        forecolor.blue = (ival<<8);
    
	if(optionflag & RIGHT_FIXATION_CHECK  && !optionflags[GREY_MONOC])
        forecolor.red = 0;
	else
        forecolor.red = (ival<<8);
    
	if(ival < 120)
        forecolor.green = (254<<8);
	else
        forecolor.green = (0<<8);
	/*	if(option2flag & PSYCHOPHYSICS_BIT)
     forecolor.red = forecolor.blue = forecolor.green;
     */
	forecolor.flags = DoRed | DoGreen | DoBlue;
	if(option2flag & PSYCHOPHYSICS_BIT || optionflags[STIMULUS_IN_OVERLAY]){
        if(ival > 0){
            forecolor.green = (ival<<8);
            forecolor.red = (ival<<8);
            forecolor.blue = (ival<<8);
            curscolor.green = (ival<<8);
            curscolor.red = (ival<<8);
            curscolor.blue = (ival<<8);
        }
        else{
            forecolor.green = (255<<8);
            forecolor.red = (255<<8);
            forecolor.blue = (255<<8);
            curscolor.green = (ival<<8);
            curscolor.red = 0;
            curscolor.blue = 0;
        }
	...
	if(optionflag & LEFT_FIXATION_CHECK && !optionflags[GREY_MONOC])
        ...
        backcolor.blue = (ival<<8);
	if(optionflag & RIGHT_FIXATION_CHECK  && !optionflags[GREY_MONOC])
        backcolor.red = (0);
	else
        backcolor.red = (ival<<8);
    
    /* changed to make background grey Apr 96 */
	backcolor.green = (ival<<8);	
	curscolor.green = forecolor.green;
	curscolor.red = forecolor.red;
	curscolor.blue = forecolor.blue;
	curscolor.flags = forecolor.flags;
	if(twomonitors){
        forecolor.red = forecolor.blue = forecolor.green = 0;
        backcolor.red = backcolor.blue = backcolor.green = (255<<8);
        forecolor.red = forecolor.blue = forecolor.green = (255<<8);
        backcolor.red = backcolor.blue = backcolor.green = 0;
        backcolor.blue = (128<<8);
	}
	XFreeColors(XtDisplay(toplevel), cmap, &forecolor.pixel, 1, 0);
	XAllocColor(XtDisplay(toplevel),cmap, &forecolor);
	if(option2flag & PSYCHOPHYSICS_BIT || optionflags[STIMULUS_IN_OVERLAY]){
        if(ival > 0){
            forecolor.green = (ival<<8);
            forecolor.red = (ival<<8);
	...
	XFreeColors(XtDisplay(toplevel), cmap, &backcolor.pixel, 1, 0);
	XAllocColor(XtDisplay(toplevel),cmap, &backcolor);
	XFreeColors(XtDisplay(toplevel), cmap, &curscolor.pixel, 1, 0);
	XAllocColor(XtDisplay(toplevel),cmap, &curscolor);
}
...
int SetStimulus(Stimulus *st, float val, int code, XEvent *event)
{
	...
	OneStim *psine;
	Substim *rds,*lrds;
	int lastflag = st->flag, ival = (int)val;
	float bval, cval, dval, left, right, eccentricity, chord_scale,phase;
	double cosa,sina,meandisp,dm,a,b,total, opos, rval, lval, odisp;
	int oldoptionflag = optionflag;
	static int laststimtype[2] = {STIM_NONE};
	static int setblank = 0, setuc = 0, setleft = 0, setright = 0, setbroad = 0;
    ...
    /*
     * if image is preloaded, its too late to do anything here....
     */
	if(val == INTERLEAVE_EXPT_HIGH && st->type == STIM_IMAGE){
        val = 0;
        if(!setbroad)
            setbroad =  st->left->orbw;
        st->left->orbw = MAXORBW*2;
        st->right->orbw = MAXORBW*2;
	}
	else if(setbroad){ // was an interleave, but not now
        st->left->orbw = setbroad;
        st->right->orbw = setbroad;
        setbroad = 0;
	}
	...
	if(val == INTERLEAVE_EXPT_BLANK){
        if(!expt.st->preload)
            st->type = STIM_NONE;
        else if (st->immode == IMAGEMODE_ORBW && st->type == STIM_IMAGE)
            st->xstate = val;
        setblank = 1;
	}
	else if(setblank && (st->immode == IMAGEMODE_ORBW && st->type == STIM_IMAGE)){
        st->xstate = 0;
        setblank = 0;
	}
	...
	switch(code)
	{
            ...
            if(st->type == STIM_RDS){
                ...
                if(!(optionflag & BACKGROUND_FIXED_BIT)){
                    if(st->next != NULL && expt.stimvals[BACK_CORRELATION] > 0)
                        SetStimulus(st->next, val, MIXAC, NULL);
                }
            }
            else
                st->corrmix = -1;
        ...
        case VSR:
            if(val < 0)
                expt.vals[VSR] = 1/(fabsf(val));
            else
                expt.vals[VSR] = (fabsf(val));
            break;
        ...
        case STIM_SIZE:
            ...
            if(pos->ss[0] <= 0)
                pos->ss[0] = 1;
            phase = pos->phase; /* don't reset with init_stim */
            init_stimulus(st);
        ...
        case ORI_BANDWIDTH:
            ...
            if (val > 200){
                printf("OB set to %.1f\n",val);
                if(seroutfile)
                    fprintf(seroutfile,"OB set to %.1f\n",val);
            }
            break;
        case ORI_LEFT:
            left = val * M_PI/180.0;
        ...
        case SF:
            st->f = pos->sf = val;
            if((st->type == STIM_GRATING || st->type == STIM_GRATING2) && val > 0.01){
                lastsf = val;
            }
            if(up){
                optimize_stimulus(st);
        ...
        case PHASE_AS_DISP:
             ...
             */	  else if(st->type == STIM_RDS && 0){
                 ...
                 expt.vals[DISP_P] = st->phasedisp[0] = val;
             }
             else{
        ...
        case ORTHOG_POSR:
            fval = GetProperty(&expt, st, PARA_POS);
            rval = GetProperty(&expt, st, ORTHOG_POSR);
            lval = GetProperty(&expt, st, ORTHOG_POSL);
            if(code == ORTHOG_POSL)
                lval = val;
            else
                rval = val;
            if( val > INTERLEAVE_EXPT){
                ...
                cosa = cos(expt.rf->angle * M_PI/180.0);
                sina = sin(expt.rf->angle * M_PI/180.0);
                bval = (opos * sina - fval * cosa);
                cval = (fval * sina + opos * cosa);
                bval =  pix2deg(expt.rf->pos[0]) - bval;
                cval +=  pix2deg(expt.rf->pos[1]);
                SetStimulus(st,bval,XPOS,event);
                SetStimulus(st,cval,YPOS,event);
                SetStimulus(st,odisp,ORTHOG_DISP,event);
            }
            break;
        case PARA_POS:
        ...
        case BACKGROUND_IMAGE:
            SetExptProperty(&expt, st, code, val);
            break;
        default:
            return(-1);
	}
	...
	{
        if(st->flag != lastflag || optionflag != oldoptionflag)
...
void handle_pushbuttons(Widget w, XtPointer data, XmPushButtonCallbackStruct *cbs)
{
	...
	switch ((int)data)
	{
        ...
        case NULL_SOFTOFF:
            sprintf(buf,"%snull\n",serial_strings[SOFTOFF_CODE]);
            for(i = 0; i < 4; i++)
                oldsoftoff[i] = expt.softoff[i];
            lasteyecheck = wurtzctr;
            SerialString(buf,0);
            break;
        case UNDO_NULL_SOFTOFF:
            for(i = 0; i < 4; i++)
                expt.softoff[i] = oldsoftoff[i];
            SerialSend(SOFTOFF_CODE);
            break;
        case COPY_ONLINE:
            sprintf(buf,"/bgc/bgc/perl/cprc %s/%s/*rc*",datprefix,expname);
            printf("%s\n",buf);
            //	  system(buf);
            break;
        case CENTRE_STIM_ON_RF:
        ...
        case SAVE_RF:
            ...
            break;
        case PAUSE_EXPT:
            states[EXPT_PAUSED] = 1;
            oldoption = optionflag;
            oldboption = option2flag;
            optionflag &= (~(STORE_WURTZ_BIT));
            option2flag &= (~(AFC));
            expt.vals[VSACCADE_VALUE] = fabsf(afc_s.sacval[1]);
            afc_s.sacval[1] = 0;
            SerialSend(OPTION_CODE);
        ...
        case REPLAY_EXPT:
            if(replay_expt){
                ...
                ReplayExpt(replay_expt);
                StopGo(GO);
            }
            break;
        case RESUME_EXPT:
            optionflag = oldoption;
            option2flag = oldboption;
            afc_s.sacval[1] = expt.vals[VSACCADE_VALUE];
            SerialSend(OPTION_CODE);
            SetAllPanel(&expt);
            states[EXPT_PAUSED] = 0;
            SerialSend(VSACCADE_VALUE);
            break;
        ...
        case QUIT_BUTTON:
            ...
            if(w == (Widget)NULL || confirm_yes("Are you sure you want to exit?",NULL)
               || what == NOCONFIRM)
                exit_program();
        case QUIT_NOCONFIRM:
            exit_program();
        ...
        case QUIT_END_PEN:
            if(penlog)
                fclose(penlog);
            if(getuid() ==343)
                system("/bgc/bgc/perl/chklogs -go");
            else
                system("su - bgc -c \"/bgc/bgc/perl/chklogs -go\"");
            exit_program();
            break;
        case STOP_BUTTON:
            /* if in an expt this button shoud do nothing */
            if(TheStim->mode & EXPTPENDING)
                break;
            mode &= (~TEST_PENDING);
            SetStimulus(TheStim,0.0,SETPHASE,NOEVENT);
            if(mode & ANIMATE_BIT)
                StopGo(STOP);
            else
                StopGo(GO);
            ...
            break;
        case UPLOAD_CODE:
            sprintf(buf,"%2s%s\n",serial_strings[(int)data],expt.upload);
            SerialString(buf,0);
            SerialSend(RF_DIMENSIONS);
            break;
        case QUERY_STATE:
            ShowTrialCount(0.0, wsum);
            sprintf(buf,"Screen %dx%d = %.2f x%.2f\n",winsiz[0] * 2, winsiz[1] * 2, pix2deg(winsiz[0] * 2),pix2deg(winsiz[1] * 2));
            acknowledge(buf,NULL);
        case SEND_CLEAR:
            SerialSend((int)(data));
            break;
...
#ifdef IRIX64
#ifdef OXFORD
  command = "ps -e|grep matlab|grep -v grep";
  p = popen(command, "r");
  n = fscanf(p, "%*s %*s %*s %s", result);
  pclose(p);
#endif
#endif
 ...
 */
	...
	initial_setup();
	n = 0;
	toplevel = XtAppInitialize(&app_context, "binoc", NULL,ZERO,
	    &argc, argv, fallback_resources, args,n);

	expt.polygonsmooth = 1;
	windowname = argv[0];
	set_alert_frame(toplevel, app_context);
	...
	SetWidgetColor(0.5);
	if(expt.winsiz[0] > 0){
	  winsiz[0]=expt.winsiz[0]/2;
	  winsiz[1]=expt.winsiz[1]/2;
	}
...
/*
	...
	while(i < argc)
	{
	  if(argv[i][0] != '-'){
		...
		if(!strncmp(buf,"monitor ",5)){
		  s = strchr(buf,'=');
		  ReadMonitorSetup(nonewline(++s));
		}
	      }
	      fclose(fd);
	    }
	    else{
	      fprintf(stderr,"Can't read %s: Check Path\n",argv[i]);
#ifdef NIDAQ
		    DIOClose();
#endif
	      exit_program();
 ...
 */
	...
	if(verbose)
	  printf("Interface....\n");
	make_interface(toplevel, TheStim);
//Ali #ifdef macosx
	n = 0;
	XtSetArg(args[n], XmNdefaultPosition, FALSE); 
	n++;
	if(winpos[0] < 50)
	  XtSetArg(args[n], XmNx, winpos[0]+(2*winsiz[0])+10); 
	else
	  XtSetArg(args[n], XmNx, winpos[0]-75); 
	n++;
	XtSetArg(args[n], XmNy, winpos[1]+15); 
	n++;
	lform = XmCreateFormDialog(toplevel, "Dummy", args, n);
	XtManageChild(lform);
	fiterate(lform, 0, 0);
	HideWidgetDecoration(lform,1);
 ...
 */
	...
	SetPanelColor(TheStim->gammaback);
	SetStimPanel(TheStim);
	if(!demomode)
	  framefront();
	setgamma(gammaval);
	...
	if(reset)
	  handle_pushbuttons(NULL, (XtPointer *) RESET_TRIALS, NULL);
	event_loop();
}
...
{
	/* XtVaSetValues(w, XmNautoUnmanage, TRUE, 0);*/
...
void forcecolor(Widget w)
{
	XtVaSetValues(w, XmNbackground, backcolor.pixel,
	    XmNforeground, forecolor.pixel,
	    XmNhighlightColor, backcolor.pixel, 
	    XmNarmColor, backcolor.pixel, 
	    XmNselectColor, forecolor.pixel, 
	    XmNtopShadowColor, forecolor.pixel, 
	    XmNbottomShadowColor, forecolor.pixel,
	    XmNtroughColor, forecolor.pixel,
	    XmNborderColor, forecolor.pixel, NULL);
}
...
void fiterate(Widget w, String att, XtArgVal val)
{
	...
	if(menu != 0)
		fiterate(menu, att, val);
	if(n > 0)
	{
		XtVaGetValues(w, XmNchildren, &l, NULL);
		for(i = 0; i < n; i++)
		{
			fiterate(l[i],att,val);
		}
...
void SetPanelColor(float color)
{
	...
	if(thecursor != (Cursor)NULL)
		XRecolorCursor(D, thecursor,&curscolor,&curscolor);
...
/*  SetWidgetTime(clockwidget);
  ...
  if(mode & HOLD_STATUS)
    return;
  if(allstatuswidget != NULL)
	my_set_label(allstatuswidget, s);
}
...
if(TheStim->mode & EXPTPENDING) /* cant play with this button  in expts */
  ...
  if((onoff == GO && monkeypress == WURTZ_STOPPED) ||
    ...
    {
	w = FindWidgetChild(form, STOP_BUTTON);
/* 
...
*/
	...
	if(onoff == GO)
	  mode &= (~ANIMATE_BIT);
	handle_pushbuttons(w,(XtPointer)STOP_BUTTON,NULL);
      }
}
...
void ButtonDrag(vcoord *start, vcoord *end, XEvent e)
{
  ...
  mpos[0] = e.xbutton.x_root - winpos[0];
  mpos[1] = e.xbutton.y_root - winpos[1];
 ...
 */
  ...
  if(eventstate & LBUTTON)
    {
      ...
      LocateStimulus(stimptr, e.xmotion.x_root-winpos[0],e.xmotion.y_root-winpos[1]);
  ...
  if(eventstate & RBUTTON)
    {
      stmode |= (MOVED_STIMULUS | DRAG_STIMULUS);
      RotateStimulus(e.xmotion.x_root-winpos[0],e.xmotion.y_root-winpos[1]);
...
void ShiftDrag(vcoord *start, vcoord *end, XEvent e)
{
  ...
  if(eventstate & LBUTTON)
    {
      Line(start[0],start[1],end[0],end[1],TheStim->gammaback);
      end[0] = mpos[0];
      end[1] = mpos[1];
      Line(start[0],start[1],end[0],end[1],1.0);
    }
  if(eventstate & MBUTTON)
    {
      StimLine(start,es,TheStim->gammaback);
      StimLine(mpos,es,BOX_COLOR);
      start[0] = e.xbutton.x_root - winpos[0];
      start[1] = e.xbutton.y_root - winpos[1];
    }
}
 ...
 */
  ...
  if(e.xbutton.button == Button2 && TheStim->type == STIM_NONE){
	RewardOn(0);
  }
}
...
void ButtonDown(vcoord *start, vcoord *end, XEvent e)
{
  ...
  mpos[0] = e.xbutton.x_root;
  mpos[1] = e.xbutton.y_root - winpos[1];
  gettimeofday(&now, NULL);
  pause = timediff(&now,&firstframetime);
//  printf("B%d at %.3f\n",e.xbutton.button,pause);
  ...
  if(eventstate & LBUTTON && mode & PLACE_PLOT_BIT)
...

	{
	  endpt[0] -= winsiz[0];
	  endpt[1] -= winsiz[1];
	  up = ButtonResponse(e.xbutton.button,0,endpt);
	  stimstate = INTERTRIAL;
	  stimstate = POSTTRIAL;
	  fixstate = RESPONDED;
    ...
    case INTERTRIAL:
      if(e.xbutton.button == Button2){
	    ...
	    if(isharris(altstimmode))
	      XWarpPointer(D, None, display_win, None, None, 0 , 0, 0,0);
      }
 ...
 */
	    {
	      if(e.xbutton.button == Button1 || e.xbutton.button == Button3){
	     	      up = ButtonResponse(e.xbutton.button,1,endpt);
	...
	{
	  stmode |= MOVED_STIMULUS;
	  LocateStimulus(stimptr, e.xbutton.x_root-winpos[0],e.xbutton.y_root - winpos[1]);
	...
	{
	  RotateStimulus(e.xbutton.x_root,e.xbutton.y_root - winpos[1]);
	  stmode |= MOVED_STIMULUS;
	}
    }
}
...
void ShiftButtonRelease(vcoord *start, vcoord *end, XEvent e)
{
  ...
  if(e.xbutton.button == Button1)
    {
      Line(start[0],start[1],end[0],end[1],expt.overlay_color);
      end[0] = mpos[0];
      end[1] = mpos[1];
      Line(start[0],start[1],end[0],end[1],LINES_COLOR);
...
*/
  i = 0;
  handle = 0;
  while(windowlist[i] != NULL)
    if(e.xany.window == windowlist[i++])
      handle = 1;

  if (!handle || handle) // do this anyway
	...
	if((sym == XK_F1 || sym == XK_space) && demomode)
	  ReadExptFile(NULL,0,0,0);
	else if(sym == XK_F1)
	  RunOneTrial();
	if(sym == XK_F2 && demomode){
	  InitExpt(1);
	  TheStim->mode |= EXPTPENDING;
	  stimstate = PREFIXATION;
	}
	if((sym == XK_F3 || sym == XK_Right) && demomode){
    if(stimstate == WAIT_FOR_RESPONSE)
      stimstate = INTERTRIAL;
		...
		if(stimno == -1)
		  stimno = 0;
		if((i = PrepareExptStim(0,7)) >= 0){
   //      TheStim->mode &= (~EXPTPENDING);
      RunExptStim(TheStim, TheStim->nframes,D,display_win);
	...
	stimstate = WAIT_FOR_RESPONSE;
      ...
      if(sym == XK_Escape && demomode)
	exit_program();
      }
    }
  if(handle && e.xany.type == KeyPress){
/*
 * Win32/MKS seems like a keypress when Alt is down does not generate an 
 * event at all.
 */
    if(e.xkey.state & Mod1Mask && e.xkey.state & ControlMask){ /* Ctrl + Alt Keys  down */
      sym = XLookupKeysym(&e.xkey,0);
      if(sym == XK_space){
	freezeexpt = !freezeexpt;
	if(freezeexpt)
	  stimno--;
      }
      if((sym == XK_plus || sym == XK_equal) && freezestimulus){
	  if(testflags[PLAYING_EXPT])
	    ReplayExpt(NULL);
      }
      if((sym == XK_plus || sym == XK_equal) && freezeexpt){
	  if(testflags[PLAYING_EXPT]){
	    ReplayExpt(NULL);
	  }
	  else
      ...
      if(sym == XK_minus && freezeexpt){
	  stimno--;
      }
    }
//Ali #if defined(WIN32) || defined(macosx)
    if(e.xkey.state & (ControlMask | Mod1Mask)) /* Ctrl Key is down */
//Ali #else
//Ali     if(e.xkey.state & Mod1Mask) /* Alt Key is down */
//Ali #endif
      {
    sym = XLookupKeysym(&e.xkey,0);
    res = handle_keypress(sym, e);
      }
    else
      {
	sym = XLookupKeysym(&e.xkey,0);
	if(sym == XK_F6)
	  SerialSignal(FREE_REWARD);
	else if(sym == XK_F1 && demomode ==0)
	  RunOneTrial();
	else{
	XLookupString(&e.xkey, cbuf, 256, NULL, NULL);
	instring[sctr++] = cbuf[0];
	if(cbuf[0] == 0)
	  sctr--;
/*
 * Don't know why this was 2. 
	else if(cbuf[0] == '\b' && sctr > 2)
*/
	else if(cbuf[0] == '\b' && sctr > 1)
	  {
	  instring[--sctr] = 0;
	  instring[--sctr] = 0;
	  }
  ...
  case CreateNotify:
    break;
  case MapNotify:
    XRecolorCursor(dpy, thecursor,&curscolor,&curscolor);
    glTranslatef(0,0,0);
    clear_overlay();
    clear_display(1);
    break;
  case Expose:
    if(!window_is_mapped && e.xany.window == display_win)
      {
	glTranslatef(0,0,0);
	init_windows();
	...
	SetExpPanel(&expt);
/*
 * guess this was to cope with files where this was not set, but it semes
 * odd, sometimes you want this set to 0 in the stimfile.
 * so removed BGC, Nov 2000
 *	expt.vals[BLANKCOLOR_CODE] = 1.0;
 */

	window_is_mapped++;
//AliGL	i = glXSwapIntervalMESA(1);
//AliGL	nf = glXGetSwapIntervalMESA();
	printf("glXSwapIntervalMESA returns %d,%f\n",i,nf);
      }
    else if(e.xany.window == display_win)
      {
	if(mode & WINDOWS_RAISED)
	  {
	    clear_display(0);
	    mode &= (~WINDOWS_RAISED);
	  }
	  ...
	  {
	    while(XCheckTypedWindowEvent(dpy, e.xany.window, Expose, &e))
	      ;
  ...
  case ButtonPress:
    ...
    if(e.xbutton.button == Button2){
      eventstate |= MBUTTON;
      if(TheStim->type == STIM_NONE && !(e.xbutton.state & ShiftMask) && 
    ...
    else if(e.xbutton.state & ControlMask)
      CntrlButtonDown(start, endpt, e);
    else
      ButtonDown(start, endpt, e);
//Ali #ifndef macosx
/*
 * in macosx can get erroneous Button4 events from a middle mouse press
 */
    if(e.xbutton.button == Button4)
...
//Ali #endif
    break;
  case MotionNotify:
    while(XCheckTypedWindowEvent(dpy, e.xany.window, MotionNotify, &e))
      ;
/*
...
*/
    ...
    else if(e.xmotion.state & ControlMask)
	  ShowPos(e.xmotion.x_root - winpos[0],e.xmotion.y_root - winpos[1]);
    else if(e.xmotion.state & ShiftMask)
	  ShowDataPos(expt.plot, e.xmotion.x_root - winpos[0],e.xmotion.y_root - winpos[1]);
    else
...
void StartRunning()
{
  ...
  if(optionflag & DITHERMODE && confirm_no("Are You Sure You want Dithering?",NULL))
  ...
  if(forcestart){
    if(replay_expt){
      SetAllPanel(&expt);
      handle_pushbuttons(NULL, (XtPointer)(REPLAY_EXPT), NULL);
    }
    else{
      if(forcestart > 1)
	set_test_loop();
      StopGo(GO);
      SetAllPanel(&expt);
      handle_pushbuttons(NULL, (XtPointer)(NULL_SOFTOFF), NULL);
    }
  }
    ...
    if(!expt.mon->loaded)
      acknowledge("Monitor File not Loaded",NULL);
...
/*
  handle_keypress is called when a key is hit while the ALT key was down.
 */
int handle_keypress(KeySym sym, XEvent e)
{
  ...
  switch(sym)
	...
	case XK_F2:
	  if(states[EXPT_PAUSED])
	    handle_pushbuttons(NULL, (XtPointer)RESUME_EXPT,NULL);
	  else
	    handle_pushbuttons(NULL, (XtPointer)PAUSE_EXPT,NULL);
	  break;
	case XK_c: /* halt expt run */
	case XK_C: /* halt expt run */
		expt_over(CANCEL_EXPT);
		break;
	case XK_e: /* Exit with no confirm if Alt_X is next */
	  c = 'e';
	  break;
	case XK_h: /* halt expt run */
	case XK_H: /* halt expt run */
		expt_over(0);
		break;
/*
 * ALT-i makes the stimulus last forever. ALT-i again toggles off.
 * shift ALT-i  actually freezes it at the current frame.
 * N.B. in physiology mode, this is not picked up until after the stimulus
 * is over, so freeze the expt first. (Cntrl-ALT-space)
 */
	case XK_i:
	  if(e.xkey.state & ShiftMask)
	    freezestimulus = 2;
	  else
	    freezestimulus = !freezestimulus;
		break;
	case XK_r: /* run expt */
		runexpt(NULL, TheStim, NULL);
		break;
	case XK_p:
		framefront();
		break;
	case XK_q:
	  SerialSend(QUERY_STATE);
		break;
	case XK_f:
		expfront();
		c = 'f';
	...
	case XK_n:
	  handle_pushbuttons(NULL, (XtPointer)NULL_SOFTOFF,NULL);
		break;
	case XK_t: // Timeout for shaking
	  if(expt.st->background > 0.0001){
	    SetStimulus(expt.st,0.0, SETBACKCOLOR,NULL);
	    StopGo(STOP);
	    sprintf(buf,"Blackout at %s",TimeString());
	    statusline(buf);
	  }
	  else{
	...
	case XK_KP_3:
	case XK_KP_Next:
		TheStim->pos.angle -= (stepsize *M_PI/360.0);
		break;
	case XK_KP_9:
	case XK_KP_Prior:
		TheStim->pos.angle += (stepsize *M_PI/360.0);
		break;
	case XK_Up:
	case XK_KP_8: /* up arrrow */
	case XK_KP_Up: /* up arrrow */
		TheStim->pos.xy[1] += stepsize;
		break;
	case XK_KP_2: /* down arrrow */
	case XK_KP_Down: /* down arrrow */
		TheStim->pos.xy[1] -= stepsize;
		break;
	case XK_KP_6: /* right arrrow */
		TheStim->pos.xy[0] += stepsize;
		break;
	case XK_KP_4: /* left arrrow */
	case XK_KP_Left: /* left arrrow */
		TheStim->pos.xy[0] -= stepsize;
		break;
	case XK_KP_Divide:
		break;
	...
	case XK_x:
	case XK_X:
		if(lastc == 'e' || confirm_yes("Are you sure you want to exit?", NULL))
		   exit_program();
		break;
    case XK_z:
      c = 'c';
      if(mode & CURSOR_IS_ON)
	{
	  mode &= ~(CURSOR_IS_ON);
			if(thecursor != (Cursor)NULL)
				XRecolorCursor(D, thecursor,&forecolor,&forecolor);
 ...
 */
	...
	if(ExptIsRunning() && (i = PrepareExptStim(1,13)) < 0){
	  if(confirmer_state == NULL || *confirmer_state == 0){
	    w = acknowledge("Error Setting Stimulus",NULL);
	    XtVaGetValues(w, XmNuserData, &confirmer_state, 0);
 ...
 */
	  ...
	  CheckKeyboard(D, allframe);
	    framesdone = RunExptStim(TheStim, TheStim->nframes, D, display_win);
#ifdef MONITOR_CLOSE
	    if(seroutfile)
 ...
 */
		...
		if(isharris(altstimmode)){
		  ...
		  XWarpPointer(D, None, display_win, None, None, 0 , 0, endpt[0], endpt[1]);
...
void frameback()
{
	if(display_win == (Window)NULL)
		makewindow(NULL);
	XLowerWindow(D,XtWindow(toplevel));
}

void framefront()
{
	XRaiseWindow(D,XtWindow(toplevel));
#ifdef Linux
	XLowerWindow(D,display_win);
#endif
}
...
{
  float wsize = 780, hsize = 600;
  unsigned int bw,depth,h,w;
  int x,y;
  Drawable root_w,win,theWindow;
  Display *dsy;
  Atom mwmHints = 0;
  MotifWmHints hints;
  int stereo = optionflags[STEREO_GLASSES];
...
/*
  w = 1280;
      h = 1024;
*/
    if(expt.winsiz[0] > 1 && expt.winsiz[1] > 1){
      winsiz[0] = expt.winsiz[0];    
      winsiz[1] = expt.winsiz[1];
    }
	if(winsiz[0] > 1 && winsiz[1] > 1){
	    w = winsiz[0] *2;	
	    h = winsiz[1] *2;	
	}
	else{
...
/*
  if(y+line[1] > winsiz[1] || y+line[1] < -winsiz[1])
    return;
*/
//Ali #ifdef macosx   // can afford to antialias all sides
      glVertex2f(x+line[0],y+line[1]);
      glVertex2f(x+line[6],y+line[7]);
//Ali #else
//Ali       glVertex2f(x+line[0],y+line[1]);
//Ali       glVertex2f(x+line[2],y+line[3]);
//Ali       glVertex2f(x+line[4],y+line[5]);
//Ali       glVertex2f(x+line[6],y+line[7]);
//Ali #endif
}
...
void SetStimPanel(Stimulus *st)
{
	...
	if(allslider != NULL)
		for(i = 0; i < allslider->nprops; i++)
			set_fpslider(allslider, allslider->proplist[i].code, StimulusProperty(st,allslider->proplist[i].code));
	SetStimPopupPanel();
}

/*
* sfpset is called when a new slider function is set
*/
void sfpset(Sliderdat *fpd)
{
	float fval;

	fpd->val = StimulusProperty(stimptr, fpd->code);
	mode |= (NEWPOS | WINDOWS_RAISED);
}

/*
* fpproc handles floats sent from the floating point sliders
*/
...
void fpproc(Fpslider *fp,float val, XEvent *event)
{
  int i;
	switch(fp->id)
	{
	case OTHERBITS_SLIDER:
		SetProperty(&expt,stimptr,fp->code,val);
	...
	case EVERYTHING_SLIDER:
	  if((i = SetStimulus(stimptr,val,fp->code,event)) < 0)
		SetProperty(&expt,stimptr,fp->code,val);
		break;
	default:
		SetProperty(&expt,stimptr,fp->id,val);
		if(event == NULL || event_is_up(event))
		  SerialSend(fp->id);
		break;
	}
...
void fnumeric_proc(Widget w, int val, int data)
{
	...
	case SET_SEEDLOOP:
	case SET_SEED:
	  SetStimulus(stimptr, (float)val, data, NULL);
		break;
	}
}
...
void SetStimPopupPanel()
{
  ...
  rds = stimptr->left;
	XtVaGetValues(lform, XmNnumChildren, &n, XmNchildren, &l, 0);
	for(i = 0; i < n; i++)
	  {
	    XtVaGetValues(l[i], XmNuserData, &what, 0);
	    if(stimptr != NULL) switch(what)
	      {
	      case SET_SEED:
		numeric_set(l[i], rds->baseseed);
		break;
	      case SET_SEEDLOOP:
		numeric_set(l[i], rds->seedloop);
		break;
	      case SF:
	      case TF:
	      case DISP_X:
	      ...
	      case DISP_P:
	      case DISP_P2:
		XtVaGetValues(l[i], XmNnumChildren, &p, XmNchildren, &m, 0);
		if(p > 0)
		  {
		  XtVaGetValues(m[0], XmNuserData, &fp, 0);
		    set_fpslider(fp, 0, GetProperty(&expt, stimptr, what));
			       }
		  break;
		}
	      }
...
void panel_popup()
{
	Widget dialog,stimtoggle_box,item,w,litem;
	Arg args[20];
	int n = 0,i = 0,x,y, j=0;
	Dimension oh;
	XmString str1;
	static int laststimtype = -1;
	float value;
	char *ctr,**stimstrings = 0, buf[BUFSIZ];
	Fpslider *fp;
	void stimtoggle_callback();

	if(stimptr->type != laststimtype && lform != NULL)
	{
		XtDestroyWidget(XtParent(lform));
		lform = NULL;
		laststimtype = stimptr->type;
	}
	if(lform != NULL)
	{
	  SetStimPanel(stimptr);
		XtManageChild(lform);
	...
	n++;
	XtSetArg(args[n], XmNy, 60); 
	n++;
	XtSetArg(args[n], XmNdefaultPosition, FALSE); 
	n++;
	lform = XmCreateFormDialog(toplevel, "Control", args, n);
	litem = my_make_button(lform,"Done", FCloseShell, NULL, 0, (int)XtParent(lform));
	item = my_make_button(lform,"Test", test_button, 
	    litem, NEXTRIGHTWIDGET_BIT,(int)XtParent(lform));
	if(stimptr->type == STIM_RDS)
	{
		stimstrings = flagstrings;
		i = (int)(StimulusProperty(stimptr, SET_SEEDLOOP));
		item = my_make_numeric(lform,"Seed Loop", fnumeric_proc, 
		    item, NEXTRIGHTWIDGET_BIT, i, 2, SET_SEEDLOOP);
		i = (int)(StimulusProperty(stimptr, SET_SEED));
		item = my_make_numeric(lform,"Seed", fnumeric_proc, 
		    item, NEXTRIGHTWIDGET_BIT, i, 3, SET_SEED);
	}
	if(stimptr->type == STIM_CYLINDER)/*j*/
	{
		stimstrings = flagstrings;
		item = my_make_numeric(lform,"Fixed Dot num ", cylinder_proc, 
		    item, NEXTRIGHTWIDGET_BIT, 100, 4, JNUMDOTS);
		item = my_make_numeric(lform,"dot life", cylinder_proc, 
		    item, NEXTRIGHTWIDGET_BIT, 10, 4, JLIFEFRAMES);
	}
	
	if(stimptr->type != STIM_CYLINDER) { /*j*/
	    value = StimulusProperty(stimptr, DISP_X);
	    fp = make_fpslider(lform, "Disparity  (deg)",-1.0,1.0,value, 2,
		litem, NEXTDOWNWIDGET_BIT, (int) 201, fpproc, DISP_X);
	    if(stimptr->type == STIM_RDS)
	    {
		    value = StimulusProperty(stimptr,DOT_DENSITY);
		    fp = make_fpslider(lform, "Dot Density     ", 0.0,100.0,value, 0,
			fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, DOT_DENSITY);
		    value = StimulusProperty(stimptr,DOT_SIZE);
		    fp = make_fpslider(lform, "Dot Size     ", 0.0,0.5,value, 3,
			fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, DOT_SIZE);
		    value = StimulusProperty(stimptr,SEED_SHIFT);
		    fp = make_fpslider(lform, "Seed Posn Shift", 0.0,5.0,value, 2,
			fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, SEED_SHIFT);
	    }
	    fp = make_fpslider(lform, "Disparity  Phase", 0.0,10.0,value, 0,
			fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, DISP_P);
    
	    if(stimptr->type == STIM_GABOR || stimptr->type == STIM_GRATING)
	    {
		    value = StimulusProperty(stimptr, SF);
		    fp = make_fpslider(lform, "S.F.       ", 0.0,30.0,value, 0,
			fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, SF);
		    value = StimulusProperty(stimptr, DISP_P);
	    }
	    if(stimptr->type == STIM_GABOR)
	    {
		    value = StimulusProperty(stimptr, SD_X);
		    fp = make_fpslider(lform, "Sigma X    ", 0.0,10.0,value, 0,
			fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, SD_X);
		    value = StimulusProperty(stimptr, SD_Y);
		    fp = make_fpslider(lform, "Sigma_Y    ", 0.0,10.0,value, 0,
			fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, SD_Y);
	    }
	    value = StimulusProperty(stimptr, TF);
	    fp = make_fpslider(lform, "T.F.       ", 0.0,10.0,value, 1,
		fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, TF);
	    value = StimulusProperty(stimptr, SETCONTRAST);
	    fp = make_fpslider(lform, "Contrast   ", -1.0,1.0,value, 2,
		fp->rc, NEXTDOWNWIDGET_BIT, (int) 201, fpproc, SETCONTRAST);
	}
	else{/*j it is a cylinder!*/
		value = StimulusProperty(stimptr, JVELOCITY);
		fp = make_fpslider(lform, "Velocity       ", 0.0,360.0,value, 0,
		    item, (NEXTDOWNWIDGET_BIT | 1), (int) 200, fpproc, JVELOCITY);
		value = StimulusProperty(stimptr, STIM_WIDTH);
		fp = make_fpslider(lform, "Height         ", 0.0,20.0,value, 1,
		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, STIM_HEIGHT);
		value = StimulusProperty(stimptr, STIM_HEIGHT);
		fp = make_fpslider(lform, "Width          ", 0.0,20.0,value, 1,
		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, STIM_WIDTH);
		value = StimulusProperty(stimptr, DISP_X);
		fp = make_fpslider(lform, "Disparity  (deg)", 0.0,1.0,value, 2,
		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, DISP_X);
		value = StimulusProperty(stimptr,DOT_DENSITY);
		fp = make_fpslider(lform, "Dot Density    ", 0.0,25.0,value, 1,
		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, DOT_DENSITY);
 		value = StimulusProperty(stimptr, STANDING_DISP);
 		fp = make_fpslider(lform, "Standing disp (deg)", 0.0,1.0,value, 2,
 		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, STANDING_DISP);
		value = StimulusProperty(stimptr,DOT_SIZE);
		fp = make_fpslider(lform, "Dot Size       ", 0.0,1.0,value, 2,
		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, DOT_SIZE);
		value = StimulusProperty(stimptr, JDEATH);
		fp = make_fpslider(lform, "DEATHchance    ", 0.0,1.0,value, 2,
		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, JDEATH);
		value = StimulusProperty(stimptr, JF_INTENSITY);
		fp = make_fpslider(lform, "Front intensity", 0.0,255.0,value, 0,
		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, JF_INTENSITY);
		value = StimulusProperty(stimptr, JB_INTENSITY);
		fp = make_fpslider(lform, "Back intensity ", 0.0,255.0,value, 0,
		    fp->rc, NEXTDOWNWIDGET_BIT, (int) 200, fpproc, JB_INTENSITY);
	}
	if(stimstrings != 0)
	{
		stimtoggle_box = XtVaCreateWidget("togglebox",
		    xmRowColumnWidgetClass, lform,
		...
		if(stimptr->type == STIM_CYLINDER)
		    ...
		    j=0;
		while(stimstrings[i] != NULL){
			w = XtVaCreateManagedWidget(stimstrings[i],
			    xmToggleButtonGadgetClass, stimtoggle_box,
			    XmNset, (stimptr->flag & (1<<j)) ? TRUE : FALSE,NULL);
			XtAddCallback(w, XmNvalueChangedCallback, (XtCallbackProc)stimtoggle_callback, (XtPointer)j);
			i++;
			j++;
		}
		XtManageChild(stimtoggle_box);
	}
...
void stimtoggle_callback(Widget item, int bit, XmToggleButtonCallbackStruct *cbs)
{
  ...
  if(bit == LAMINARTRODE_TOGGLE){
	...
	if(cbs->set)
		...
		stimptr->flag &= ~(1<<bit);
	if(rdspair(stimptr) || (stimptr->next != NULL && stimptr->next->type == STIM_RDS))
	  stimptr->next->flag = stimptr->flag;
    SerialSend(OPTION_CODE);
}
...
void update_option_display()
{
  ...
  if(otoggle_box == NULL)
    return;
  XtVaGetValues(otoggle_box, XmNnumChildren, &n, XmNchildren, &l, 0);
  for(i = 0; i < 32; i++){
    XtVaSetValues(l[i],
		  XmNset, (optionflag & (1<<i)) ? TRUE : FALSE,NULL);
  }
  for(i = 32; i < MAXOPTIONBITS; i++){
    XtVaSetValues(l[i],
		  XmNset, (option2flag & (1<<(i-32))) ? TRUE : FALSE,NULL);
  }
  for(i = MAXOPTIONBITS; i <= MAXOPTIONBITS+LAST_CODED_OPTION; i++){
    XtVaSetValues(l[i],
 ...
 */
  ...
  if((new | gone) & MONOCULAR_MODE && thecursor != 0)
    {
      SetWidgetColor(TheStim->gammaback);
      XRecolorCursor(D, thecursor,&curscolor,&curscolor);
    }
...
void toggle_callback(Widget item, int bit, XmToggleButtonCallbackStruct *cbs)
{
  char buf[BUFSIZ];
  int flag;
  if(bit >= MAXOPTIONBITS){
    flag = bit-MAXOPTIONBITS;
    optionflags[flag] = cbs->set;
    CheckOption(bit);
  }
  else if(bit >= 32)
    {
	...
	switch((1<<bit))
	{
	case PSYCHOPHYSICS_BIT:
		setoption();
		clear_display(1);
		break;
	      case INTERLEAVE_UNCORRELATED:
	      case INTERLEAVE_MONOCULAR:
	      case INTERLEAVE_ONE_MONOCULAR:
	      case INTERLEAVE_BLANK:
		setoption();
		break;
	case PERF_STRING:
		if(!(cbs->set)){
	...
	case GO_BIT:
	  if(optionflag & GO_BIT){
	    ...
	    expt.vals[ACQUISITION_CODE] = 1.0;
	    if(!(TheStim->mode & EXPTPENDING) && optionflags[STORE_EXPT_ONLY]
	       && optionflag & STORE_WURTZ_BIT){
	      optionflag &= (~STORE_WURTZ_BIT);
	      nostore = 1;
	    }
	    ...
	    StopGo(STOP);
	    if(!(TheStim->mode & EXPTPENDING) && optionflags[STORE_EXPT_ONLY] && nostore == 1){
	      optionflag |= (STORE_WURTZ_BIT);
	      nostore = 0;
	    }
	  }
	...
	case SQUARE_RDS:
	  if(cbs->set)
	    TheStim->flag |= STIMULUS_IS_SQUARE;
	  else
	    TheStim->flag &= ~(STIMULUS_IS_SQUARE);
	  break;
	case FRAME_ONLY_BIT:
	...
	case DITHERMODE:
	  if(cbs->set)
	    ...
	    glDisable(GL_DITHER);
		break;
	case WAIT_FOR_BW_BIT:
	if(optionflag & WAIT_FOR_BW_BIT && !(mode & SERIAL_OK))
		MakeConnection();
	break;
	case CLAMP_EXPT_BIT:
		SerialSend(CLAMP_DISPARITY_CODE);
		break;
	case RAMP_HOLD_BIT:
	  if(cbs->set)
	    optionflag |= RAMP_EXPT_BIT;
	  else
	    optionflag &= (~RAMP_EXPT_BIT);
		SerialSend(RAMP_HOLD_CODE);
		SerialSend(RAMP_VERGENCE_CODE);
		break;
	case RAMP_EXPT_BIT:
		SerialSend(RAMP_VERGENCE_CODE);
		break;
	case STORE_WURTZ_BIT:
	    nostore = 0;
		SetRunButton(NULL);
		break;
      case RIGHT_FIXATION_CHECK:
      case LEFT_FIXATION_CHECK:
		setoption();
		clear_display(1);
		break;
     }
...
void load_file(Widget fs, XtPointer client_data,
XmFileSelectionBoxCallbackStruct *cbs)
{
	...
	if((i = ReadExptFile(filename,1,1,1)) >= 0)
	{
...
void SaveExpt(Widget fs, XtPointer client_data,
XmFileSelectionBoxCallbackStruct *cbs)
{
...
{
	void load_file(),FCloseShell(),SaveExpt();
	XmString save;

	if(sfsdialog == NULL)
	{
	sfsdialog = XmCreateFileSelectionDialog(parent, "filesb", NULL, 0);
	save = XmStringCreateSimple("Save");
	XtVaSetValues(sfsdialog, XmNokLabelString, save, NULL);
	XmStringFree(save);
	XtAddCallback(sfsdialog, XmNokCallback, (XtCallbackProc)SaveExpt, NULL);
	XtAddCallback(sfsdialog, XmNcancelCallback, (XtCallbackProc)FCloseShell, (XtPointer)XtParent(sfsdialog));
	XtManageChild(sfsdialog);
	if(loadfiles[0] && *loadfiles[0])
	  {
	    save = XmStringCreateSimple(loadfiles[0]);
	    XtVaSetValues(sfsdialog, XmNdirSpec, save, 0);
	    XmStringFree(save);
	  }
	}
	fiterate(sfsdialog, 0, 0);
	XtManageChild(sfsdialog);
	XtPopup(XtParent(sfsdialog),XtGrabNone);
}
...
void read_file_popup(Widget parent, char *arg)
{
	...
	if(fsdialog == NULL)
	{
	fsdialog = XmCreateFileSelectionDialog(parent, "filesb", NULL, 0);
	save = XmStringCreateSimple("Load");
	XtVaSetValues(fsdialog, XmNokLabelString, save, NULL);
	XmStringFree(save);
	XtAddCallback(fsdialog, XmNokCallback, (XtCallbackProc)load_file, NULL);
	XtAddCallback(fsdialog, XmNcancelCallback, (XtCallbackProc)FCloseShell, (XtPointer)XtParent(fsdialog));
	XtManageChild(fsdialog);
	if(loadfiles[0] && *loadfiles[0])
	  {
	    save = XmStringCreateSimple(loadfiles[0]);
	    XtVaSetValues(fsdialog, XmNdirSpec, save, 0);
	    XmStringFree(save);
	  }
	}
	fiterate(fsdialog, 0, 0);
	XtManageChild(fsdialog);
	XtPopup(XtParent(fsdialog),XtGrabNone);
}
...
void set_stimulus(Widget w, char *type)
{
	...
	int i = -1,j,k;
	float val;
	char cbuf[256],buf[BUFSIZ];
	struct stat statbuf;
	Winloc wp;
	wp.x = wp.y = 0;

	if(strncasecmp(type,"gabor",4) == 0)
	{
		i = StimulusType(stimptr,STIM_GABOR);
		paint_stimulus(stimptr);
	}
	else if(strncasecmp(type,"sine",4) == 0)
		i = StimulusType(stimptr,STIM_GRATING);
	else if((!strncasecmp(type,"dblsine",4)) || (!strncasecmp(type,"2xsine",4)))
		i = StimulusType(stimptr,STIM_GRATING2);
	else if(strncasecmp(type,"rds",3) == 0)
	{
		i = StimulusType(stimptr,STIM_RDS);
		/*		paint_stimulus(stimptr);*/
	}
	else if(strncasecmp(type,"probe",3) == 0)
		i = StimulusType(stimptr,STIM_PROBE);
	else if(strncasecmp(type,"bar",3) == 0)
		i = StimulusType(stimptr,STIM_BAR);
	else if(strncasecmp(type,"squ",2) == 0)
		i = StimulusType(stimptr,STIM_SQUARE);
	else if(strncasecmp(type,"cylinder",3) == 0)
		i = StimulusType(stimptr,STIM_CYLINDER);
	else if(strncasecmp(type,"none",4) == 0)
		i = StimulusType(stimptr,STIM_NONE);
	else if(strncasecmp(type,"test",4) == 0)
	{
		/*     StimulusType(stimptr,STIM_TEST);*/
		test_proc(TheStim);
		return;
	}
	else if(strncmp(type,"optimize",6) == 0)
		optimize_stimulus(TheStim);
	else if(strncmp(type,"options",4) == 0)
	  PopupOptionPanel();
	else if(strncmp(type,"panel",4) == 0)
		panel_popup();
	else if(strncmp(type,"expt",4) == 0)
		exp_popup(toplevel, TheStim, thecursor);
	else if(strncmp(type,"wurtz",4) == 0)
		wurtz_popup(toplevel, TheStim, thecursor);
	else if(strncmp(type,"afc",4) == 0)
	  afc_popup(toplevel, TheStim, thecursor,wp);
	else if(strncmp(type,"bw",2) == 0)
	  bw_popup(toplevel, thecursor,wp);
	else if(strncmp(type,"log",3) == 0)
		log_popup(toplevel,TheStim, thecursor);
	else if(strncmp(type,"unit",3) == 0)
		unit_popup(toplevel,TheStim, thecursor);
	else if(strncmp(type,"load",4) == 0)
		read_file_popup(toplevel,NULL);
	else if(strncmp(type,"Wexp",4) == 0){
	  sscanf(type,"Wexp%d",&i);
	  sprintf(cbuf,"./q%dexp",i);
	  SaveExptFile(cbuf,QUICK_SAVE);
	  
	  if((k = stat("./exphist",&statbuf)) == 0){ // exists
	  j = 1;
	  do{
		sprintf(buf,"./exphist/%s.qe%d",expname,j++);
	  }while((k = stat(buf,&statbuf)) == 0);
	  sprintf(cbuf,"cp ./q%dexp %s",i,buf);
	  system(cbuf);
	}
	  make_quickmenu();
	}
	else if(strncmp(type,"Write",4) == 0)
		save_file_popup(toplevel,NULL);
	else if(strncmp(type,"save",4) == 0)
	  {
		if(loadfiles[0] && *loadfiles[0])
			SaveExptFile(loadfiles[0],0);
		sprintf(cbuf,"Setup Saved in %s",loadfiles[0]);
		statusline(cbuf);
	  }
	if(i == 0){
		mode |= NEWPOS;
...
void set_run(Widget w, char *type)
{
	...
	if(strncmp(type,"cont",4) == 0)
	{
		mode |= ANIMATE_BIT | NEWPOS;
		clear_display(1);
		  TheStim->fix.state = FIX_IS_UP;
		monkeypress = -1;
		resetframectr();
	}
	else if(strncmp(type,"restart",5) == 0)
	{
		framecount = 0;
	}
	else if(strncmp(type,"reset",5) == 0)
	{

		XDestroyWindow(D,display_win);
		display_win = 0;
		makewindow(NULL);
		return;
	}
	...
	else if(strncmp(type,"freeze",5) == 0)
	  {
		mode &= (~ANIMATE_BIT);
		stepframe = 0;
	      }
	else if(strncmp(type,"kill",5) == 0)
	{
		XDestroyWindow(D,display_win);
		display_win = 0;
		mode &= (~ANIMATE_BIT);
...
void plotexptbutton(Widget w, Stimulus *st, XmAnyCallbackStruct *cbs)
{
	...
	setmask(OVERLAY);
	SetGrey(TheStim->gammaback);
	myrect(expt.plot->pos[0]-100,expt.plot->pos[1]-50,expt.plot->pos[0]+expt.plot->size[0]+202,expt.plot->pos[1]+expt.plot->size[1]+50);
	SetColor(PLOT_COLOR,1);
       	if(optionflags[PLOT_ELECTRODE_TRACK])
	  plottrack(plot);
	else
	  plotexpt(expt.plot);
...
void makeRasterFont(void)
{
	...
	"-adobe-helvetica-medium-r-normal--12-120-*-*-p-*-*-1");
    if (fontInfo == NULL) {

    fontInfo = XLoadQueryFont(D, 
	"-adobe-helvetica-medium-r-normal--14-120-100-100-p-88-iso8859-1");
    }
...
//AliGL    glXUseXFont(id, first, last-first+1, base+first);


    mediumbase = base;
/*
 * Build a medium font list
 */

    fontInfo = XLoadQueryFont(D, 
	"-adobe-helvetica-medium-r-normal--18-120-*-*-p-*-*-1");
    if (fontInfo != NULL) {
      medid = fontInfo->fid;
      medfirst = fontInfo->min_char_or_byte2;
      medlast = fontInfo->max_char_or_byte2;
    mediumbase = glGenLists((GLuint) medlast+1);
    if (mediumbase == 0) {
        printf ("out of display lists\n");
	exit (0);
    }
//AliGL    glXUseXFont(medid, medfirst, medlast-medfirst+1, mediumbase+medfirst);

    }


/*
 * Build a large font list
 */
    fontInfo = XLoadQueryFont(D, 
	"-adobe-helvetica-medium-r-normal--48-120-*-*-p-*-*-1");
    if (fontInfo == NULL) {

    fontInfo = XLoadQueryFont(D, 
	"-adobe-helvetica-medium-r-normal--48-120-100-100-p-88-iso8859-1");
    }
    ...
    bigid = fontInfo->fid;
    bigfirst = fontInfo->min_char_or_byte2;
    biglast = fontInfo->max_char_or_byte2;

    bigbase = glGenLists((GLuint) biglast+1);
    if (bigbase == 0) {
        printf ("out of display lists\n");
	return;
    }
//AliGL    glXUseXFont(bigid, bigfirst, biglast-bigfirst+1, bigbase+bigfirst);
...
void expt_over(int flag)
{
  ...
  if(mode & MORE_PENDING && mode & AUTO_NEXT_EXPT)
      ...
      if((i = ReadExptFile(NULL, 0,0,0)) >= 0)
	{
	  ...
	  if(flag == CANCEL_EXPT  && confirm_no("Run Next expt?",NULL))
